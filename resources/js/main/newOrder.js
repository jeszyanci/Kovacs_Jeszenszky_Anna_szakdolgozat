...
let orderElem = new Order();
let tempFile  = null;

function createNewOrderForm(elem, data=false, _tempFile=false) {
    if (data)
        orderElem = data;
    ...
    // ORDERFORM ELEMENT
    let orderForm = $(`
        <div class="orderForm printableContent"></div>
    `).appendTo(mainHolder);
    ...
    let buyerElement = $(`
        <div style="display:flex; flex-direction:column; align-items: center; margin-bottom: 10px;">
            <div class="box" style="display:flex; justify-content:center; text-align:center; margin-bottom: 10px; width:100%;">
                <div style="flex:1;">${data ? pad(orderElem.id, 8) : ''}</div>
            ...
            <div class="box" style="width:60%; text-align:center;">
                ...
                <div style="display:flex; flex-direction:row; align-items: center;">
                    <div class="formblockHolder" style="padding:10px;">
                        <div class="formBlock">
                            <span style="display:inline-block; width:150px; text-align:right;">Név</span>
                            <input param="buyername" type="text" value="${data ? data.buyername : ''}" style="width: 200px; height: 30px;">
                        </div>
                        <div class="formBlock">
                            <span style="display:inline-block; width:150px; text-align:right;">Telefonszám</span>
                            <input param="buyerphone" type="text" value="${data ? data.buyerphone : ''}" style="width: 200px; height: 30px;">
                        </div>
                        <div class="formBlock">
                    ...
                    <div class="formblockHolder_company">
                        <div class="formBlock">
                            <span style="display:inline-block; width:150px; text-align:right;">Cég név</span>
                            <input param="companyname" type="text" value="${data ? data.companyname : ''}" style="width: 200px; height: 30px;">
                        </div>
                        <div class="formBlock">
                            <span style="display:inline-block; width:150px; text-align:right;">Telefonszám</span>
                            <input param="companyphone" type="text" value="${data ? data.companyphone : ''}" style="width: 200px; height: 30px;">
                        </div>
                        <div class="formBlock">
                            <span style="display:inline-block; width:150px; text-align:right;">E-mail</span>
                            <input param="companyemail" type="text" value="${data ? data.companyemail : ''}" style="width: 200px; height: 30px;">
                        </div>
                    </div>
                </div>
            </div>
    ...
    ['buyername', 'buyerphone'].forEach( paramName => {
        buyerElement.find(`input[param="${paramName}"]`).on('change', event => {
            orderElem[paramName] = $(event.target).val().trim();
        });
    });
    ...
    let gotMaterialElement = $(`
        <div style="display:flex; flex-direction:row; gap:1%;">
            <div id="gotMaterial" class="formblockHolder box" style="flex:1; display:flex; flex-direction:column; align-items: center; margin-bottom: 10px; width:30%">
            ...
            <div id="orderData" class="box" style="flex:3;"></div>
        </div>
    `).appendTo(orderForm);
    ...
    $(`[openPopupMaterial]`).on('click', () => {
        if ( !$(`.popup`).length ) {
            createMaterialPopup(orderElem.details.gotMaterials);
        }
    });
    
    let orderElement = $(`
        <div class="block">
            <div class="boxTitle">Ékszer</div>
            <div style="width:100%; display:flex; flex-direction: row; padding:5px;" class="formblockHolder">
                <div style="width:100%;" class="formblockHolder">
                    <div class="formBlock">
                        <span class="dataLabel" style="display:inline-block; width:130px;">Szélesség</span>
                        <input productWidth type="text" style="width:200px;" ${data ? `value="${data.details.product.sizes.width.value}"` : ''}>
                        <select unitW>
                            ${["mm", "cm"].map( unit => `<option value="${unit}">${unit}</option>`).join('')}
                        </select>
                    </div>
                    <div class="formBlock">
                        <span class="dataLabel" style="display:inline-block; width:130px;">Magasság</span>
                        <input productHeight type="text" style="width:200px;" ${data ? `value="${data.details.product.sizes.height.value}"` : ''}>
                        <select unitH>
                            ${["mm", "cm"].map( unit => `<option value="${unit}">${unit}</option>`).join('')}
                        </select>
                    </div>
                    <div class="formBlock">
                        <span class="dataLabel" style="display:inline-block; width:130px;">Hosszúság</span>
                        <input productLength type="text" style="width:200px;" ${data ? `value="${data.details.product.sizes.length.value}"` : ''}>
                        <select unitL>
                            ${["mm", "cm"].map( unit => `<option value="${unit}">${unit}</option>`).join('')}
                        </select>
                    </div>
                ...
                <div style="width:100%;" class="formblockHolder">
                    <div class="formBlock">
                        <span class="dataLabel" style="display:inline-block; width:130px;">Szín</span>
                        <select productColor style="width:200px;" name="dropdown">
                            ${[
                                ["s#", "s# (sárga)"],
                                ["f#", "f# (fehér)"],
                                ["v#", "v# (vörös)"],
                                ...
                                <option value="${type[0]}" ${data && data.details.product.color == type[0] ? `selected` : `` }>${type[1]}</option>
                            `).join('') }
                        </select>
                    </div>
                    
                    <div class="formBlock">
                        <span class="dataLabel" style="display:inline-block; width:130px;">Súly</span>
                        <input productWeight type="text" style="width:200px;" ${data ? `value="${data.details.product.weight}"` : ''}>
                        <!-- ex. 10 - 13 -->
                        <span>gramm</span> 
                    </div>

                    <div class="formBlock">
                        <span class="dataLabel" style="display:inline-block; width:130px;">Ujjméret</span>
                        <input fingerSize type="text" style="width:200px;" ${data ? `value="${data.details.product.fingerSize}"` : ''}>
                    </div>
                </div>
            </div>
        </div>
    `)
    .appendTo(orderForm.find('#orderData'));
    
    // order data
    orderForm.find('[productColor]').on('change', event => {
        orderElem.details.product.color = $(event.target).val();
    });
    
    orderForm.find('[productWeight]').on('change', event => {
        orderElem.details.product.weight = $(event.target).val();
    });
    
    orderForm.find('[fingerSize]').on('change', event => {
        orderElem.details.product.fingerSize = $(event.target).val();
    });
    
    orderForm.find('[productWidth]').on('change', event => {
        orderElem.details.product.sizes.width.value = $(event.target).val();
    });
    
    orderForm.find('[productHeight]').on('change', event => {
        orderElem.details.product.sizes.height.value = $(event.target).val();
    });
    
    orderForm.find('[productLength]').on('change', event => {
        orderElem.details.product.sizes.length.value = $(event.target).val();
    });
    
    // units
    orderForm.find(`[unitW]`).on('change', event => {
        orderElem.details.product.sizes.width.unit = $(event.target).val();
    });
    
    orderForm.find(`[unitH]`).on('change', event => {
        orderElem.details.product.sizes.height.unit = $(event.target).val();
    });
    
    orderForm.find(`[unitL]`).on('change', event => {
        orderElem.details.product.sizes.length.unit = $(event.target).val();
    })
    ...
    extraElement.find('[description]').on('change', event => {
        orderElem.details.description = $(event.target).val();
    });

    extraElement.find('#sketch').on('change', event => {
        orderElem.details.sketch = event.target.files.length > 0;
    ...
    ['currency', 'deposit', 'price'].forEach( dataName => {
        priceElems.find(`[${dataName}]`).on('change', event => {
            orderElem.priceData[dataName] = $(event.target).val();
        });
    ...
    deadlineElem.find('#deadline').on('change', event => {
        orderElem.deadline = $(event.target).val();
    });
}
...
function createTable() {
    ...
    let methods = orderElem.details.methods;
    if (!methods)
        return;
    ...
    methods.forEach( (method, i) => {
        let methodData = METHOD_TYPES.find( type => type.id == method.id);

        sumTime += Number(method.time);
        guidePrice += Number(methodData.hourly_wage) * Number(method.time);
    ...
    orderElem.priceData.price = guidePrice + servicePrice;
...
let createMethodPopup = () => {
                ...
                <div class="formBlock">
                    <div>
                        ...
                        <select methodSelector class="dropdown" style="display: inline-block; font-size: 15px">
                            ${METHOD_TYPES.map( type => `<option value="${type.id}">${type.name}</option>`).join('') }
                        </select>
                    </div>
    ...
    popup.find('[addMethod]').on('click', () => {
        ...
        orderElem.details.methods.push({
            id:   type,
            time: time,
            name: name
        });
        ...
        let hours = 0;
        orderElem.details.methods.forEach( method => {
...
let refreshAddedMats = () => {
    ...
    orderElem.details?.gotMaterials?.forEach( (material, i) => {
        let li = $(`
        ...
        li.find('[removeBtn]').click( e => {
            orderElem.details?.gotMaterials?.splice(i, 1);
            refreshAddedMats();
        });
    });
};
        ...
        <section>
            <div>
                ...
                <div class="" style="display: flex; flex-direction: column;">
                    ...
                    <div>Megrendelő:</div>
                        <div>Dátum: ${orderElem.date}</div>
                        <div class="formBlock">
                            <span>Név: ${orderElem.buyername}</span>
                        </div>
                        <div class="formBlock">
                            <span>Tel: ${orderElem.buyerphone}</span>
                        </div>
                    </div>
                    <div>Leírás: ${orderElem.details.description}</div>
                    <div>Hozott anyagok:</div>
                    <div>
                        <ul>
                            ${orderElem.details.gotMaterials.map( mat => `<li>${mat.weight} gramm - ${mat.name} ${mat.type != 'other' ? `(${mat.type})` : ``}</li>`).join('')}
                        </ul>
                    </div>
                    <div>Eljárások:</div>
                    <div>
                        <table>
                            ...
                            ${orderElem.details.methods.map( method => `<tr><td>${method.name}</td><td>${method.time} óra</td></tr>`).join('')}
                        </table>
                    </div>
                    <div>Megrendelés:</div>
                    <div>
                        <div>Szín: ${orderElem.details.product.color}</div>
                        <div>Súly: ${orderElem.details.product.weight} gramm</div>
                        <div>Ujjméret: ${orderElem.details.product.fingerSize}</div>
                        <div>
                            <div>Szélesség: ${orderElem.details.product.sizes.width.value} ${orderElem.details.product.sizes.width.unit}</div>
                            <div>Magasság: ${orderElem.details.product.sizes.height.value} ${orderElem.details.product.sizes.height.unit}</div>
                            <div>Hosszúság: ${orderElem.details.product.sizes.length.value} ${orderElem.details.product.sizes.length.unit}</div>
                        </div>
                    </div>
                    <div>Előleg és irányár:</div>
                    <div>
                        <div>Előleg: ${orderElem.priceData.deposit} ${orderElem.priceData.currency}</div>
                        <div>Irányár: ${orderElem.priceData.price} ${orderElem.priceData.currency}</div>
                    </div>
                </div>
            </div>
        </section>
